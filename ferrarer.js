// ferrarer.js - Ferrarer-automallin 3D-määritelmä (Siro viistoitettu keula)
(function() {
  ...
  window.CAR_MODELS['ferrarer'] = function(bodyColorHex, accentColorHex, carTexUrl) {
    ...
    // --- Ferrarer: Kaareva ja reunoiltaan sulavasti viistoitettu konepelti ---
    var hoodGeo = new THREE.BoxGeometry(1.58, 0.34, 1.3);
    var pos = hoodGeo.attributes.position;
    for (var i = 0; i < pos.count; i++) {
      var z = pos.getZ(i);
      if (z > 0) { // Etuosa
        var factor = z / 0.65;
        pos.setX(i, pos.getX(i) * (1.0 - factor * 0.32)); // Kulmakapeus
        pos.setY(i, pos.getY(i) - factor * 0.09);         // Keulan madallus
      }
    }
    hoodGeo.computeVertexNormals();
    hoodGeo.translate(0, 0.38, 1.25);
    ...
    hl1.rotation.y = -0.18;
    hl1.position.set(-0.48, 0.44, 1.48);
    var hl2 = hl1.clone();
    hl2.rotation.y = 0.18;
    hl2.position.x = 0.48;
    ...
    spot1.position.set(-0.48, 0.44, 1.52);
    ...
    spot2.position.set(0.48, 0.44, 1.52);

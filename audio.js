...
(function() {
  ...
  function setupCarEngineSound(carObj) {
    ...
    // 1. Tyhjäkäynti / Rullaus / Maksiminopeus (moottori.mp3, loop)
    var idleAudio = new Audio('moottori.mp3');
    idleAudio.loop = true;
    idleAudio.volume = 0;

    // 2. Kiihdytys (kiihdytys.mp3, soitetaan kiihdytyksessä)
  ...
  function updateCarEngineSound(carObj, isGas, speed, maxSpeed) {
    ...
    // Lasketut sävelkorkeus- / toistonopeus-kertoimet suhteessa maksiminopeuteen
    ...
    if (!isGas) {
      // Ei kaasua: toistetaan moottori.mp3 rullauksena, toistonopeus nopeuden mukaan
      ...
      // Kaasu pohjassa JA maksiminopeudessa: nopeutettu moottori.mp3 loopilla
      if (carObj.accelAudio) { carObj.accelAudio.pause(); }
      ...
      // Kaasua painetaan ja kiihdytetään: kiihdytys.mp3

...
(function() {
  ...
  /* ---------------------------------------------------------------
     HIEKKAPÖLY-, VESI- JA TÖRMÄYSKIPINÄPARTIKKELIT
  --------------------------------------------------------------- */
  var dustParticles = [];
  ...
  // TÖRMÄYSKIPINÄT (Pienet keltaiset pallot)
  var sparkParticles = [];
  ...
  function buildCarMesh(bodyColorHex, accentColorHex, carTexUrl, modelType) {
    ...
    // Kiihdyttimen hohto-mesh (glow aura)
      ...
      if (c) {
        ...
        if (c.finished) {
          ...
          if (speedometerEnabled) {
            ...
            var barHue = Math.round(120 * (1 - speedRatio)); // Vihreästä punaiselle
            html += '<div class="speedometer-widget" style="margin-top:8px; padding-top:6px; border-top:1px solid rgba(0,0,0,0.1); text-align:center;">';
            html += '<div style="font-size:1.6rem; font-weight:900; font-family:Consolas, monospace; color:#1e293b;">' + kmh + ' <span style="font-size:0.75rem; font-weight:700;">km/h</span></div>';
            html += '<div style="width:100%; height:6px; background:#e2e8f0; border-radius:3px; overflow:hidden; margin-top:3px;">';
            html += '<div style="width:' + Math.round(speedRatio * 100) + '%; height:100%; background:hsl(' + barHue + ', 85%, 45%); transition:width 0.1s linear;"></div>';
            html += '</div></div>';
          }
        }
  ...
  function updatePhysics(delta){
    ...
    for(var i = 0; i < cars.length; i++) {
      ...
      // KIIHDYTTIMEN HOHTOEFEKTIN ANIMAATIO

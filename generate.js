// generate.js - Radan ja maaston generointimoottori
(function() {
  'use strict';
  ...
  function buildTrackPath() {
    ...
    // SIIRRETÄÄN MAALIVIIVA KAUAS SILLASTA
    var samples = [];
    var shiftOffset = 0;
    if (overpassCenter !== -1) {
  ...
  function buildPitStop(track, tireWearEnabled) {
    var pGroup = new THREE.Group();
    var pitStopArea = null;
    if (!track || !tireWearEnabled) return { group: pGroup, pitStopArea: pitStopArea };
    ...
    var perp = new THREE.Vector3(-s.tz, 0, s.tx).normalize();
    var sideDist = ROAD_HALF_WIDTH + 1.8;
    ...
    var pitTex = new THREE.CanvasTexture(c);
    ...
    pitStopArea = { x: px, z: pz, radius: 4.2 };
  ...
  function buildTerrain(track, currentEnvironment, currentSeason, texturesEnabled, loadTextureWithFallback, ENV_TEXTURE_PATHS) {
    ...
    var terrainTex = texturesEnabled ? loadTextureWithFallback(groundTexUrl, size / 6, size / 6, '#3a6628', 'MAA') : null;
  ...
  function buildRoad(track, texturesEnabled, loadTextureWithFallback, ENV_TEXTURE_PATHS, currentEnvironment) {
    ...
    var asphaltTexUrl = (currentEnvironment === 'hitech') ? ENV_TEXTURE_PATHS.hitechroad : ENV_TEXTURE_PATHS.asphalt;
    var asphaltTex = texturesEnabled ? loadTextureWithFallback(asphaltTexUrl, 1, 30, '#333333', 'RATA') : makeAsphaltProceduralTex();
